...
let globalEmitter = new Emitter();

export const setNaukarReady = (_naukar) => {
  naukar = _naukar;
  globalEmitter.emit('ready', undefined);
};

export const setNaukarDestroy = () => {
  naukar = undefined;
  globalEmitter.destroy();
  globalEmitter = new Emitter();
};

const injectAbortable = (emitter) => {
  ...
  return mainInjectAbortableProxy(injectWaitOnWorkerReadyProxy);
};

// a proxy to the worker entry, will stall any methods
// until the worker is marked ready.
export const naukarProxy = {
  current: injectAbortable(globalEmitter),
};
