...
describe('reloadUiAfterSaveDrain', () => {
  function makeEditorEngine(initiallyDirty: boolean) {
  ...
  test('keeps the tab mounted when its editor cannot drain', async () => {
    vi.useFakeTimers();
    const { reloadUiAfterSaveDrain } = await import('../main');
    const editorEngine = makeEditorEngine(true);
    const logger = { error: vi.fn(), info: vi.fn() };
    const reload = vi.fn();

    const result = reloadUiAfterSaveDrain({
      editorEngine,
      logger,
      reload,
      timeoutMs: 100,
  ...
  test('reloads as soon as this tab finishes its pending save', async () => {
    const { reloadUiAfterSaveDrain } = await import('../main');
    const editorEngine = makeEditorEngine(true);
    const logger = { error: vi.fn(), info: vi.fn() };
    const reload = vi.fn();
