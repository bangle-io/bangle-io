...
async function verifyPermission(rootDirHandle: FileSystemDirectoryHandle) {
  const permission = await hasPermission(rootDirHandle);
...
function resolveFileHandle({
  ...
  return async (
    ...
    const parentHandles: FileSystemDirectoryHandle[] = [];
...
export async function pickADirectory() {
  try {
    ...
    const permission = await requestNativeBrowserFSPermission(dirHandle);
